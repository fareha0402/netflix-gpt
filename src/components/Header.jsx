...
function Header() {
  ...
  return (
    <div className="absolute bg-gradient-to-b z-10  from-black  md:w-screen flex flex-col md:flex-row justify-between w-full">
      <img className="md:m-4 w-[150px] mx-auto md:mx-0" src={LOGO_URL}></img>
      {user && (
        <div className="flex justify-center md:p-3">
          {showLangOption && (
            <select
              className="p-2 mx-2 my-4 md:m-5 md:px-4 rounded-md bg-gray-500 text-white"
              onChange={handleSelectedLanguage}
            >
              {SUPPORTED_LANG.map((selected) => (
                <option key={selected.name} value={selected.name}>
                  {selected.value}
                </option>
              ))}
            </select>
          ...
          <button
            className="p-2 mx-2 my-4 bg-orange-400 rounded-lg text-white cursor-pointer"
            onClick={showGPTSearch}
          >
            {showLangOption ? "Home Page" : "GPT Search"}
          </button>
          <img className="md:w-10 h-10 mt-4" src={user.photoURL}></img>
          <button
